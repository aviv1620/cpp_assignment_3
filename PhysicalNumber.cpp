...
bool ariel::operator==(const ariel::PhysicalNumber & a, const ariel::PhysicalNumber & b) {//equal
	...
	return std::abs((a.value * Enumber[a.unit]) - (b.value * Enumber[b.unit])) <= std::numeric_limits<long double>::epsilon() * std::abs((a.value * Enumber[a.unit]) + (b.value * Enumber[b.unit])) * 100;// long double is not fully accurate.
}
